...
public class activity_resault_p extends AppCompatActivity { //검색된 pension 데이터 출력 코드
    ListAdapter adapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_resault_p);
        ListView lv =(ListView) findViewById(R.id.listView_main_list); //리스트뷰와 엑티비티에 리스트뷰 연결
        try{
            adapter = new SimpleAdapter( //어뎁터 설정
                    activity_resault_p.this, MainActivity.al, R.layout.item_list_p, //연결할 엑티비티 선택 al 동적배열에 담긴 데이터 사용
